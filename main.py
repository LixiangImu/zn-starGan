...
def main(config):
    ...
    if config.dataset in ['VOC', 'Both']:
        voc_loader = get_loader(config.voc_image_dir, None, None,
                               config.voc_crop_size, config.image_size, config.batch_size,
                               'VOC', config.mode, config.num_workers)
    ...
    solver = Solver(voc_loader, rafd_loader, config)

    if config.mode == 'train':
        if config.dataset in ['VOC', 'RaFD']:
            solver.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='VOC', choices=['VOC', 'RaFD', 'Both'])
    ...
    parser.add_argument('--voc_crop_size', type=int, default=256, help='crop size for the VOC dataset')
    parser.add_argument('--voc_image_dir', type=str, default='VOCdevkit')

    # Evaluation metrics
    parser.add_argument('--eval_metrics', type=str2bool, default=True, help='whether to compute evaluation metrics')
